...
EVENT_SM_DASHBOARD_NOTIFICATIONS_UPDATED = "sm_dashboard_notifications_updated"
...
@callback
@websocket_api.websocket_command({vol.Required("type"): "sm_dashboard_notification/get"})
def websocket_get_notifications(
    hass: HomeAssistant,
    connection: websocket_api.ActiveConnection,
    msg: Mapping[str, Any],
) -> None:
    """Return a list of sm_dashboard_notifications."""
...
def notifications(hass, name):
    ...
    """Set up the sm dashboard notification component."""
    
    sm_dashboard_notifications: MutableMapping[str, MutableMapping] = OrderedDict()
    hass.data[DOMAIN]["notifications"] = sm_dashboard_notifications
    ...
    def create_service(call):
        ...
        sm_dashboard_notifications[entity_id] = {
            ATTR_MESSAGE: message,
            ATTR_NOTIFICATION_ID: notification_id,
        ...
        hass.bus.async_fire(EVENT_SM_DASHBOARD_NOTIFICATIONS_UPDATED)
    ...
    def dismiss_service(call):
        ...
        if entity_id not in sm_dashboard_notifications:
            return

        hass.states.async_remove(entity_id)

        del sm_dashboard_notifications[entity_id]
        hass.bus.async_fire(EVENT_SM_DASHBOARD_NOTIFICATIONS_UPDATED)
    ...
    def mark_read_service(call):
        ...
        if entity_id not in sm_dashboard_notifications:
            _LOGGER.error(
                "Marking sm dashboard_notification read failed: "
                "Notification ID %s not found.",
                notification_id,
            )
            return

        sm_dashboard_notifications[entity_id][ATTR_STATUS] = STATUS_READ
        hass.bus.async_fire(EVENT_SM_DASHBOARD_NOTIFICATIONS_UPDATED)
