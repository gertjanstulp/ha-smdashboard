...
def load_yamll(fname, secrets=None, args={}):
    try:
        ...
        if _process_yaml:
            _LOGGER.info("Load SMD yaml: %s", fname)
            stream = io.StringIO(jinja.get_template(fname).render({
                **args,
                "_smd_global": smd_global,
                "_smd_translations": smd_translations,
                "_hass_resources": hass_resources,
                "_smd_icons": smd_icons,
                "_smd_paths": smd_paths,
            }))
            stream.name = fname
...
def _include_yaml(ldr, node):
    ...
    fn = fn.replace("__ROOT__", smd_global["rootdir"])
...
async def async_process_yaml(hass, entry):
    ...
    async def handle_reload(call):
        ...
        _LOGGER.info("Reload SMD Configuration")

        await async_reload_configuration(hass)

    # Register service smd.reload
